...
def solve(env: PullCubeToolEnv, seed=None, debug=False, vis=False):
    ...
    grasp_pose = env.agent.build_grasp_pose(
        approaching, closing, env.l_shape_tool.pose.sp.p
    )
    ...
    approach_offset = sapien.Pose(
        [-(env.hook_length + env.cube_half_size + 0.08), -0.0, lift_height - 0.05]
    )
    ...
    # -------------------------------------------------------------------------- #
    behind_offset = sapien.Pose([-(env.hook_length + env.cube_half_size), -0.067, 0])
